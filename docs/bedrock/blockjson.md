...
그냥 대충  
"자 이제부터 니 오른쪽 텍스처는 dirt.png를 쓰고 이거엔 3이라는 값을 써"  
해서 데이터를 형식에 맞게 저장해두면 그 데이터값을 불러와서  
리소스 팩으로 불러와주는 방식이다.  
어렵다고 생각될 수도 있는데  
알맞은 폴더위치에 텍스처를 저장해두면 그 텍스처 파일을 불러와서  
...
|alpha_test|완전히 투명한 텍스처에 사용. AO는 같이 비활성화 됨|
|blend     |반투명이나 완전히 투명하게가 텍스처에서 사용할 수 있게할때 씀. AO는 같이 비활성화 됨|
...
### textures
텍스처를 뭐쓸지 지정해주는 역할이다.  
이걸로 여러 블록이 같은 텍스처쓰게해서  
용량 최적화 하는것도 가능하다.  

텍스처는 파일 이름 뒤에 .png만 때주면 된다.  
다만 커스텀 텍스처는 안돼고 이미 있는 텍스처의 한에서만 가능하다.
...
```json {7-8}
{
  ...
  "leaves": {
    "textures": "concrete" // 이렇게 하면 나뭇잎에 콘크리트 텍스처 쓰게 하는거임
  }
}
```

블록에 따라 여러 텍스처를 쓰는 경우에는
